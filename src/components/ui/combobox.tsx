      ...
      <Button variant="outline" {...props} ref={ref}>
        {children ?? (
          <span className="flex w-full items-center justify-between gap-2">
            {value ? data.find((item) => item.value === value)?.label : `Seleccionar ${type}...`}
...
export const ComboboxEmpty = ({ children, ...props }: ComboboxEmptyProps) => {
  ...
  return <CommandEmpty {...props}>{children ?? `No se encontró ${type}.`}</CommandEmpty>;
};
...
export const ComboboxCreateNew = ({ onCreateNew, children, className }: ComboboxCreateNewProps) => {
  const { inputValue, onValueChange, onOpenChange } = useContext(ComboboxContext);
