...
export const Dropzone = ({
  ...
  const hasFiles = Array.isArray(src) && src.length > 0;
    ...
    <div className={cn('flex flex-col items-center justify-center', className)}>
      ...
      <p className="my-2 w-full truncate font-medium text-sm">
        {src.length > maxLabelItems
          ? `${new Intl.ListFormat('es-MX').format(
              src.slice(0, maxLabelItems).map((file) => file.name)
            )} y ${src.length - maxLabelItems} más`
          : new Intl.ListFormat('es-MX').format(src.map((file) => file.name))}
      </p>
      <p className="w-full text-wrap text-muted-foreground text-xs">
        Arrastra y suelta o haz clic para reemplazar
      </p>
    </div>
  );
};
...
export const DropzoneEmptyState = ({ children, className }: DropzoneEmptyStateProps) => {
  ...
  if (accept) {
    caption += 'Acepta ';
    caption += new Intl.ListFormat('es-MX').format(Object.keys(accept));
  }
